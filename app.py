...
def create_llms_model():
    llm = CTransformers(
        ...
        config={"max_new_tokens": 512, "temperature": 0.01, "gpu_layers": 10},
    )
    return llm
...
st.set_page_config(
    page_title="QuantQuips", page_icon="chart_with_upwards_trend", layout="wide"
)
# Set page title
st.title("QuantQuips")
st.markdown(
...
def fetch_realtime_stock_data(ticker_symbol, period, interval):
    ...
    try:
        ...
        if "15m data not available" in str(e):
            st.warning(
                f"15-minute data not available for the specified period. Fetching hourly data instead."
            )
...
nse_stock_data = fetch_realtime_stock_data(
    nse_ticker_symbol, default_period, default_interval
)

# Fetch and display real-time stock data for Sensex
sensex_ticker_symbol = "^BSESN"
sensex_stock_data = fetch_realtime_stock_data(
    sensex_ticker_symbol, default_period, default_interval
)

# Initialize percentage changes
nse_percentage_change = 0.0
sensex_percentage_change = 0.0

# Calculate the percentage change for NSE safely
if not nse_stock_data.empty:
    # yfinance 1.3.0 returns MultiIndex columns by default.
    # We extract the 'Close' column for the specific ticker.
    close_series = (
        nse_stock_data["Close"][nse_ticker_symbol]
        if isinstance(nse_stock_data["Close"], pd.DataFrame)
        else nse_stock_data["Close"]
    )
    nse_percentage_change = float(
        (close_series.iloc[-1] - close_series.iloc[0]) / close_series.iloc[0] * 100
    )

# Calculate the percentage change for Sensex safely
if not sensex_stock_data.empty:
    close_series = (
        sensex_stock_data["Close"][sensex_ticker_symbol]
        if isinstance(sensex_stock_data["Close"], pd.DataFrame)
        else sensex_stock_data["Close"]
    )
    sensex_percentage_change = float(
        (close_series.iloc[-1] - close_series.iloc[0]) / close_series.iloc[0] * 100
    )
...
overall_market_condition = (
    "Bullish"
    if nse_percentage_change > 0 and sensex_percentage_change > 0
    else "Bearish"
)
...
def plot_chart(stock_data, title, subheader):
    ...
    st.write(plot_df.tail())  # Display the last rows of the DataFrame
...
if page_selection == "Home":
    ...
    if not nse_stock_data.empty and not sensex_stock_data.empty:
        col1, col2 = st.columns(2)

        with col1:
            plot_chart(
                nse_stock_data, "Real-time Nifty Chart", "Real-time Nifty Chart:"
            )

        with col2:
            plot_chart(
                sensex_stock_data, "Real-time Sensex Chart", "Real-time Sensex Chart:"
            )
    else:
        st.warning(
            "Nifty or Sensex market is closed. Real-time data is available only during market hours."
        )
    ...
    strategy_code = st.text_area(
        "Enter your Python code:", value="", height=None, max_chars=None, key=None
    )

    st.header("Parameter Ranges")
    short_period_range = st.slider(
        "Short Period Range", min_value=10, max_value=50, value=(15, 30)
    )
    long_period_range = st.slider(
        "Long Period Range", min_value=40, max_value=100, value=(40, 60)
    )
    ...
    with container:
        with st.form(key="my_form", clear_on_submit=True):
            user_input = st.text_input(
                "Question:", placeholder="Ask about your portfolio", key="input"
            )
...
## ABOUT US
    ...
    st.markdown(
        "<style>h3{color: orange; text-align: center;}</style>", unsafe_allow_html=True
    )
    st.subheader("Our Team")
    st.markdown(
        """<div style="color: white; font-size: 24px; font-weight: bold;">Meet our team of talented individuals who are dedicated to making a difference in the fintech world.</div>
    """,
        unsafe_allow_html=True,
    )
    ...
    with col1:
        st.image(
            "https://avatars.githubusercontent.com/u/45279662?v=4",
    ...
    def run_trader(principal_amount):
        try:
            result = subprocess.run(
                ["python", "trader.py", str(principal_amount)],
                capture_output=True,
                text=True,
                check=True,
            )
    ...
    principal_amount = st.number_input(
        "Enter initial principal amount:", value=1000000000.0
    )
